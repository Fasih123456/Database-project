...
def button_equal():
    second_nummber = e.get()
    e.delete(0, END)
    if math == "addition":
        e.insert(0, f_num + int(second_nummber))
    if math == "subtraction":
        e.insert(0, f_num - int(second_nummber))
...
button_0 = Button(root, text="0", padx=40, pady=20, command=lambda: button_click()).grid(row=4, column=0)
...
button_clear = Button(root, text="Clear", padx=79, pady=20, command=button_clear).grid(row=4, column=1,
                                                                                       columnspan=2)
button_equal = Button(root, text="=", padx=91, pady=20, command=button_equal).grid(row=5, column=1,
                                                                                   columnspan=2)
